...
//
//// ============================================================
////  Per-aircraft simulation timing.
////  GTM modes are ~10x faster than CivilAircraft, so it needs
////  shorter sim windows with much finer dt to keep RK4 stable.
//// ============================================================
//struct SimTiming {
//    double nl_length;   int nl_steps;
//    double long_length; int long_steps;
//    double lat_length;  int lat_steps;
//};
//
//inline SimTiming getSimTiming(AircraftType type)
//{
//    if (type == AircraftType::NasaGTM) {
//        return { 5.0,  1000,    // nl:    dt = 0.005 s
//                10.0,  2000,    // long:  dt = 0.005 s
//                15.0,  3000 };  // lat:   dt = 0.005 s
//    }
//    return     { 150.0,  200,    // nl:    dt = 0.75  s
//               150.0,  150,    // long:  dt = 1.0   s
//               200.0,  250 };  // lat:   dt = 0.8   s
//}
//
//
...
//
//    // ----- CONFIG -----
//    AircraftType ac_type = AircraftType::NasaGTM;        // or AircraftType::CivilAircraft
//    AttitudeMode att_mode = AttitudeMode::Quaternion;
//
//    constexpr double TRIM_VA = 85.0;
//    constexpr double TRIM_H = 3048.0;
//    constexpr double TRIM_GAM = 15 * constants::DEG_TO_RAD;   // +6 deg climb
//
...
//
//    // ----- Build aircraft with a reasonable starting state -----
//    //   (the trim solver will replace these values; we just need
//    //    something to construct the aircraft object with)
//    Eigen::VectorXd Z0 = defaultTrimInitialGuess(TRIM_VA, TRIM_H);
...
//
//    std::unique_ptr<AircraftModel> ac = makeAircraft(ac_type, guessX, guessU);
//    std::cout << "Aircraft model: " << ac->name() << "\n";
//    std::cout << "Trim altitude:   " << TRIM_H << " m\n";
//    std::cout << "Speed of sound:  " << atm.speedOfSound(TRIM_H) << " m/s\n";
//    std::cout << "Air density:     " << atm.airDensity(TRIM_H) << " kg/m^3\n";
//    std::cout << "Gravity:         " << atm.gravity(TRIM_H) << " m/s^2\n";
//
//    // ----- Solve trim FIRST -----
//    TrimResult trim = solveTrim(*ac, atm, TRIM_VA, TRIM_H,TRIM_GAM);
//    if (!trim.converged) {
//        std::cerr << "ERROR: trim did not converge. "
//            << "Try tightening the initial guess via solveTrim(...,Z0,...).\n";
//        return 1;
//    }
//
...
//
//    verifyStraightLevelTrimConditions(*ac, *kin, trim_states, trim_controls, atm,TRIM_VA,TRIM_H,TRIM_GAM);
//
//    // ----- Per-aircraft timing -----
//    const SimTiming timing = getSimTiming(ac_type);
//
//    // ----- Nonlinear sim: trim point + small pitch perturbation -----
//    Eigen::VectorXd ic_euler = trim_states_euler;
//    ic_euler[7] += 0.01;     // ~0.57 deg pitch perturbation to excite the modes
//    Eigen::VectorXd ic = buildFullState(ic_euler, *kin);
//
//    try {
//        Eigen::MatrixXd NL_sol = runNonlinearSimulation(
//            *ac, *kin, ic, trim_controls, atm, timing.nl_length, timing.nl_steps);
//        outputToFileWithHeaders(NL_sol, "Non Linear Solution.csv");
//        std::cout << "Nonlinear simulation complete ("
//            << timing.nl_length << " s, " << timing.nl_steps << " steps, dt = "
//            << timing.nl_length / timing.nl_steps << " s).\n";
//
//        Eigen::MatrixXd PP = postProcessNonlinearSolution(
//            NL_sol, *kin, atm, timing.nl_length, timing.nl_steps);
//        writePostProcessCsv(PP, "postProcessSolution.csv");
//    }
//    catch (const std::exception& e) {
//        std::cerr << "WARNING: nonlinear sim failed: " << e.what() << "\n";
//        std::cerr << "         Continuing to linearization "
//            "(which doesn't time-integrate).\n";
//    }
//
...
//            timing.long_length, timing.long_steps, long_ctrl,
...
//            timing.lat_length, timing.lat_steps, lat_ctrl,
